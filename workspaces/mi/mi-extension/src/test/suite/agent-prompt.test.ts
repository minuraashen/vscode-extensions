...
import { getSystemPrompt } from '../../ai-features/agent-mode/agents/main/system';
...
suite('Agent Prompt', () => {
    ...
    test('getSystemPrompt includes semantic section when enabled', () => {
        const prompt = getSystemPrompt(undefined, true);
        assert.ok(prompt.includes('## Semantic search'));
    });

    test('getSystemPrompt removes semantic section when disabled', () => {
