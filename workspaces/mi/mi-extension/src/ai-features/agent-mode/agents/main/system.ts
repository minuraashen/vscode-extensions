...
- IMPORTANT: Use ${SEMANTIC_SEARCH_TOOL_NAME} as the MANDATORY FIRST STEP for ANY task that requires understanding or navigating the project — including answering questions, explaining code, finding artifacts, diagnosing issues, or planning changes. It returns actual source code snippets inline — make decisions directly from the returned content WITHOUT calling ${FILE_READ_TOOL_NAME} afterwards. Only use ${FILE_READ_TOOL_NAME} or grep in these specific cases: (a) ${SEMANTIC_SEARCH_TOOL_NAME} returns CONFIDENCE: LOW, (b) ${SEMANTIC_SEARCH_TOOL_NAME} returns 0 results or is unavailable, (c) you already know the exact file path and need its full content to perform an edit, or (d) the task is a targeted line-range read after semantic search has already identified the file.
